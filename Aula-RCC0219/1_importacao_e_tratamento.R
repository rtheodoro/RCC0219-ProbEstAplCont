...
names(survAluno) <- c(
   ...
   "n_pessoas_moradia",
   "entidade_estudantil",
   "tipo_entidade_estudantil",
   "religiao",
   "animais_domesticos",
   "outro_animais_domesticos",
   "n_filhos",
   "idade",
   "doa_sangue",
   "frequencia_esporte",
   ...
   "escolaridade_pai",
   "escolaridade_mae",
   "ajuda_financeira_pais",
   "transporte_em_ribeirao",
   "graduacao_anterior",
   "curso_graduacao_anterior",
   "empregado",
   "modalidade_emprego",
   "profissao",
   "trabalho_contabil",
...
survAluno <- survAluno |>
   dplyr::mutate(
      estetica_outros = ifelse(
         is.na(estetica_outros) | # ifelse(padrao, substituir X, substitui Y)
